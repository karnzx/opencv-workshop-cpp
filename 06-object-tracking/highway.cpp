...
int main(int argc, char** argv)
{
    ...
    int totalDown = 0;
    // Threshold
    const Point LINETL = Point(0, 240);
    const Point LINEBR = Point(800, LINETL.y + 5);
    const int MIN_DISTANCE = 100;   // min distance to know its same object
    const int MIN_RECT_SIZE = 350; // min object to detect
    ...
    for (;;)
    {
        ...
        cvtColor(frame, fgMaskMOG2, COLOR_BGR2GRAY);
        ...
        imshow("before contours", fgMaskMOG2);
        ...
        for (int i = 0; i < contours.size(); i++)
        {
            ...
            Rect boundRect;
            approxPolyDP(contours[i], contours_poly, 0.1 * arcLength(contours[i], true), true);
            boundRect = boundingRect(Mat(contours_poly));
            centerObj.x = boundRect.x + boundRect.width / 2;
            centerObj.y = boundRect.y + boundRect.height / 2;
            if (boundRect.area() >= MIN_RECT_SIZE)
            {
                TrackObject obj;
        ...
        for (int i = 0; i < trackObjects.size(); i++)
        {
            ...
            if (!object.counted && object.isOverlap(lineRect.tl(), lineRect.br(), objectRect.tl(), objectRect.br()))
            {
                ...
                Mat counted_car = frame(object.boundRect);
                resize(counted_car, counted_car, Size(400, 300));
                imshow("counted car", counted_car);
                trackObjects[i].counted = true;
            }
        }

        putText(frame, "^ totalUp: " + to_string(totalUp), Point(lineRect.br().x / 2 - 30, lineRect.tl().y - 10), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0, 50, 255), 2);
        putText(frame, "v totalDown: " + to_string(totalDown), Point(lineRect.br().x / 2 - 30, lineRect.tl().y + 35), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(50, 255, 50), 2);
